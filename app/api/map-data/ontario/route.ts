...
export async function GET() {
  ...
  if (!response.ok) return new Response("Unable to load Ontario boundary", { status: 502 });
  const data = await response.json();
  const features = data.features.filter((feature: { properties?: { admin?: string; name_en?: string } }) => feature.properties?.admin === "Canada" && feature.properties?.name_en === "Ontario");
